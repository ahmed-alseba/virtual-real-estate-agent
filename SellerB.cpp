...
#define PORT4 "4144" // the port client will be connecting to  
#define BACKLOG 7
int main()
{
	ifstream sellerFile;
	sellerFile.open("sellerB.txt");
	int listingPrice = 0;
	int squareFootage = 0;
	...
	cout << "SellerB has TCP port " << sellerPort << " and IP address " << sellerIP << " for Phase 1 part 1\n";
	cout << "SellerB is now connected to the Agent1\n";
	...
	cout << "SellerB has sent list price of " << listingPrice << " and square footage of " << squareFootage << " to the agent\n";

	close(sockfd);
	cout << "End of Phase 1 part 1 for SellerB\n";
	...
	cout << "SellerB has TCP port " << PORT4 << " and IP address " << inet_ntoa(*addr_list[0]) << " for Phase 1 part 4\n";
	...
	{  // main accept() loop
		...
		if (new_fd == -1) {
			...
			cout << "End of Phase 1 part 4 for SellerB\n";
			close(new_fd);
